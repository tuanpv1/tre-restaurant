    ...
    class Bookings_List_Table extends WP_List_Table
    {
        ...
        public function get_columns()
        {
            ...
            $columns = array(
                ...
                'note' => __('Note', BOOKACTI_PLUGIN_NAME),
        ...
        public function get_booking_list_items()
        {
            ...
            if ($this->filters['event_id'] && !$this->filters['event_group_id']) {
                $this->filters['booking_group_id'] = 'none';
            }
            if (!$this->filters['booking_group_id'] && $this->filters['group_by'] !== 'none') {
                $this->filters['group_by'] = 'booking_group';
            }
            ...
            foreach ($bookings as $booking) {
                if ($booking->user_id && is_numeric($booking->user_id) && !in_array($booking->user_id, $this->user_ids, true)) {
                    $this->user_ids[] = $booking->user_id;
                }
                if ($booking->group_id && !in_array($booking->group_id, $this->group_ids, true)) {
                    $this->group_ids[] = $booking->group_id;
                }
            }
            ...
            foreach ($bookings as $booking) {
                ...
                if ($booking->group_id && $may_have_groups && !$single_only) {
                    ...
                    if (isset($displayed_groups[$booking->group_id])
                        || empty($booking_groups[$booking->group_id])
                    ) {
                        continue;
                    }
                ...
                if (!empty($users[$user_id])) {
                    ...
                    $note = !empty($user->note) ? $user->note : '';
                    ...
                    $note = !empty($booking_meta->note) ? $booking_meta->note : '';
                    // Any other cases
                } else {
                    $user = null;
            ...
            foreach ($booking_list_items as $booking_id => $booking_list_item) {
                if (empty($booking_list_item['refund_actions']) && isset($booking_list_item['actions']['refund'])) {
                    unset($booking_list_item['actions']['refund']);
                }
                if (empty($booking_list_item['actions'])) {
                    continue;
                }
        ...
        public function format_filters($filters = array())
        {
            ...
            if (!$filters) {
                ...
                $event_start = '';
                $event_end = '';
                if (isset($_REQUEST['bookacti_group_id']) && $_REQUEST['bookacti_group_id'] !== 'single') {
                    $event_group_id = intval($_REQUEST['bookacti_group_id']);
                }
                if (isset($_REQUEST['event_group_id']) && $_REQUEST['event_group_id'] !== 'single') {
                    $event_group_id = intval($_REQUEST['event_group_id']);
                }
                if ($event_group_id === 0) {
                    if (isset($_REQUEST['bookacti_event_id'])) {
                        $event_id = intval($_REQUEST['bookacti_event_id']);
                    }
                    if (isset($_REQUEST['event_id'])) {
                        $event_id = intval($_REQUEST['event_id']);
                    }
                    if (isset($_REQUEST['bookacti_event_start'])) {
                        $event_start = bookacti_sanitize_datetime($_REQUEST['bookacti_event_start']);
                    }
                    if (isset($_REQUEST['event_start'])) {
                        $event_start = bookacti_sanitize_datetime($_REQUEST['event_start']);
                    }
                    if (isset($_REQUEST['bookacti_event_end'])) {
                        $event_end = bookacti_sanitize_datetime($_REQUEST['bookacti_event_end']);
                    }
                    if (isset($_REQUEST['event_end'])) {
                        $event_end = bookacti_sanitize_datetime($_REQUEST['event_end']);
                    }
                }
        ...
        protected function pagination($which)
        {
            ...
            ?>
            <form action='<?php echo esc_url(add_query_arg('paged', '%d')); ?>'
                  class='bookacti-list-table-go-to-page-form'>
                ...
                <?php if ($this->has_items()) { ?>
                    <div class='alignleft actions bulkactions'>
                        <form method='post' class='bookacti-bookings-bulk-action'>
                            <input type='hidden' name='page' value='bookacti_bookings'/>
                            <input type='hidden' name='nonce_bookings_bulk_action'
                                   value='<?php echo wp_create_nonce('bulk-' . $this->_args['plural']); ?>'/>
        ...
        {
            if (empty($_REQUEST['nonce_bookings_bulk_action'])) {
                return;
            }
