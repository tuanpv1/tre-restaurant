(function ($){
    $(document).ready( function () {
        ...
        $(document).on('click', '.br_grid_list_page_add', function(event) {
            event.preventDefault();
            if( $('.id_'+$('.br_grid_list_page_select').val()).length == 0 ) {
                var html = '<li class="br_grid_list_page_id id_'+$('.br_grid_list_page_select').val()+'"><input type="hidden" name="br_lgv_buttons_page_option[pages]['+$('.br_grid_list_page_select').val()+']" value="'+$('.br_grid_list_style_select').val()+'"><button type="button" class="button br_grid_list_page_remove">'+$('.br_grid_list_page_select').find('option:selected').text()+': '+$('.br_grid_list_style_select').find('option:selected').text()+'</button></li>';
