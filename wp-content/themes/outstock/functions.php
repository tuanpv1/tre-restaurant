...
if (!empty($_POST['submit_book'])) {
    ...
    // process time date booking
    $date_time_book = $date_book . " " . $time_book . ":00";
    $date_time_book = strtotime(str_replace('/', '-', $date_time_book));
    $date_book_start = date('Y-m-d H:i:s', $date_time_book);
    $date_book_end = date('Y-m-d H:i:s', $date_time_book + 7200); // Plus 2 hour to end booking

    // process even_id have 2 even lunch id 8 and dinner id 9
    $hour = 0;
    if (!empty($time_book)) {
        $hourA = explode(':', $time_book);
        $hour = $hourA[0];
    }
    if ($hour <= 14) {
        $even_id = 8; // hard code
    ...
    try {
        $table_bookings = $wpdb->prefix . 'bookacti_bookings';
        $data = array(
            'user_id' => $email,
            'order_id' => null,
            'form_id' => 4,
            'group_id' => null,
            'event_id' => $even_id,
            'event_start' => $date_book_start,
            'event_end' => $date_book_end,
            'state' => 'pending',
            'payment_status' => 'none',
        ...
        $format = array('%s', '%s', '%d', '%s', '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%d', '%d');
        $wpdb->insert($table_bookings, $data, $format);
        $id_booking = $wpdb->insert_id;

        $table_booking_meta = $wpdb->prefix . 'bookacti_meta';
        $arrayInfo = array(
            'user_email' => $email,
            'user_first_name' => null,
            'user_last_name' => $full_name,
            'user_phone' => $phone,
            'note' => $content_message
        );
        foreach ($arrayInfo as $key => $value) {
            $dataMeta = array(
                'object_type' => 'booking',
                'object_id' => $id_booking,
                'meta_key' => $key,
                'meta_value' => $value,
            );
            $wpdb->insert($table_booking_meta, $dataMeta);
        }
        $wpdb->query('COMMIT');
